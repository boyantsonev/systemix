...
async function doctor() {
  ...
  if (fs.existsSync(systemixDir) && fs.statSync(systemixDir).isDirectory()) {
    ...
    results.push({ icon: "❌", label: ".systemix/ directory — not found, run: npx @getsystemix/cli init", level: "error" });
  }
  ...
  if (fileKey) {
    ...
    results.push({ icon: "❌", label: "project-context.json — missing figma.fileKey, run: npx @getsystemix/cli init", level: "error" });
  } else {
    results.push({ icon: "❌", label: "project-context.json — not found, run: npx @getsystemix/cli init", level: "error" });
  }
  ...
  if (installedSkills.length === REQUIRED_SKILLS.length) {
    ...
    results.push({
      icon: "⚠️ ",
      label: `${installedSkills.length}/${REQUIRED_SKILLS.length} skills installed — missing: ${missingSkills.join(", ")}, run: npx @getsystemix/cli add figma-to-code`,
      level: "warn"
    });
  } else {
