 ...
 * `npx @getsystemix/cli init` (ADR-008). This is the Orchestrator's boot read: surfaces,
...
function validateConfig(cfg) {
  ...
  if (missing.length) {
    throw new Error(
      ...
      `Re-run \`npx @getsystemix/cli init\` to regenerate it.`
    );
  }
...
function loadConfig(projectRoot) {
  ...
  if (!fs.existsSync(p)) {
    throw new Error(`No ${CONFIG_FILE} found in this repo. Run \`npx @getsystemix/cli init\` to create an instance.`);
  }
