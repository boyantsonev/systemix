...
function show() {
  if (!fs.existsSync(DESIGN_DIR)) {
    ...
    console.log("  → existing repo:  npx systemix audit    (then /design-audit)");
    console.log("  → from scratch:   npx systemix design init  (then /design-interview)\n");
    return;
  }
...
function init() {
  ...
  console.log("  Already have UI? Use `npx systemix audit` instead.\n");
}
