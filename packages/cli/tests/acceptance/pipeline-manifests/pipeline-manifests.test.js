 ...
 *     `npx @getsystemix/cli add <pipeline>` (which reads manifest.skills then copies each
...
describe("pipeline manifests match the skills on disk", () => {
  ...
  it.each(pipelines)(
    "%s — every manifest skill exists on disk (npx @getsystemix/cli add can't break)",
    (p) => {
