...
import { BookCallLink, InstallCommand, SectionTrack } from "@/components/systemix/LandingEvents";
      ...
      <div className="mx-auto flex h-12 max-w-5xl items-center gap-6 rounded-full border border-border/50 bg-background/80 px-5 shadow-[var(--shadow-sm)] backdrop-blur-md">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" asChild>
            <BookCallLink href={nav.cta.href} source="nav">
              {nav.cta.label}
            </BookCallLink>
          </Button>
        </div>
    ...
    <Section>
      <div className="max-w-xl">
        ...
        <div className="mt-5 flex items-center gap-5">
          {about.links.map((l) => {
            const linkCls =
              "text-[13px] text-muted-foreground/60 underline underline-offset-4 transition-colors hover:text-foreground";
            return l.href.startsWith("mailto:") ? (
              <BookCallLink key={l.href} href={l.href} source="about" className={linkCls}>
                {l.label}
              </BookCallLink>
            ) : (
        ...
        {services.tiers.map((t) => (
          ...
          >
            ...
            {(() => {
              ...
              return t.cta.href.startsWith("mailto:") ? (
                <BookCallLink href={t.cta.href} source="services" className={ctaCls}>
                  {t.cta.label} →
                </BookCallLink>
              ) : (
                <a href={t.cta.href} className={ctaCls}>
                  {t.cta.label} →
                </a>
              );
            })()}
