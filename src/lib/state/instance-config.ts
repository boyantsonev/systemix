...
/**
 * Reads systemix.config.yaml — the instance topology written by `npx @getsystemix/cli init`
...
/** Instance vocabulary for the Workflow Atlas (consumed by `npx @getsystemix/cli atlas build`
...
export function serializeInstanceConfig(cfg: InstanceConfig): string {
  const lines: string[] = [
    ...
    "# Edit in the Config layer (/config) or re-run `npx @getsystemix/cli init` to regenerate.",
